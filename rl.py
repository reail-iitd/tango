...
def run_new_plan(model, init_graphs, all_actions):
	...
	while True:
		...
		a = np.random.choice(possible_actions, p=probs.detach().numpy()); p.append(probs[possible_actions.index(a)])
...
if __name__ == '__main__':
	...
	while True:
		...
		for _ in range(10):
			...
			loss.backward()
			optimizer.step()
			print("Loss =", loss.item(), " Value Loss =", avg(val_loss).item(), " Policy Loss =", avg(p_loss).item())
