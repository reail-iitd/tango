...
class GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_10_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_10_Action, self).__init__()
        self.name = "GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_10_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.activation = nn.PReLU()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
