...
class GGCN_Metric_Attn_Aseq_L_Auto_Tool_Goal_Cons_C_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_Metric_Attn_Aseq_L_Auto_Tool_Goal_Cons_C_Action, self).__init__()
        self.name = "GGCN_Metric_Attn_Aseq_L_Auto_Tool_Goal_Cons_C_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.activation = nn.PReLU()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
        ...
        self.fc2 = nn.Linear(n_hidden, n_hidden)
        self.fc3 = nn.Linear(n_hidden, len(possibleActions))
        self.p1_object  = nn.Linear(n_hidden*5 + len(possibleActions) + 1 + 1, n_hidden)
        self.p2_object  = nn.Linear(n_hidden, n_hidden)
        self.p3_object  = nn.Linear(n_hidden, 1)
        self.q1_object  = nn.Linear(n_hidden*5 + len(possibleActions) + 1 + 1 + 1, n_hidden)
        self.q2_object  = nn.Linear(n_hidden, n_hidden)
        self.q3_object  = nn.Linear(n_hidden, 1)
        self.q1_state  = nn.Linear(n_hidden*4 + len(possibleActions), n_hidden)
        self.q2_state  = nn.Linear(n_hidden, n_hidden)
    ...
    def forward(self, g_list, goalVec, goalObjectsVec, a_list,  object_likelihoods):
        ...
        for ind,g in enumerate(g_list):
            ...
            pred1_input = torch.cat([final_to_decode, one_hot_action], 1)
            pred1_object = self.activation(self.p1_object(
                        torch.cat([pred1_input.view(-1).repeat(self.n_objects).view(self.n_objects, -1), self.activation(self.embed(self.object_vec)), object_likelihoods[ind].view(self.n_objects, -1), goal_data], 1)))
            pred1_object = self.activation(self.p2_object(pred1_object))
            pred1_object = self.p3_object(pred1_object)
            pred1_output = torch.sigmoid(pred1_object)
            ...
            pred2_input = torch.cat([final_to_decode, one_hot_action], 1)
            pred2_object = self.activation(self.q1_object(
                        torch.cat([pred2_input.view(-1).repeat(self.n_objects).view(self.n_objects, -1), self.activation(self.embed(self.object_vec)), object_likelihoods[ind].view(self.n_objects, -1), pred1_output.view(self.n_objects, 1), goal_data], 1)))
            pred2_object = self.activation(self.q2_object(pred2_object))
            pred2_object = self.q3_object(pred2_object)
            pred2_object = torch.sigmoid(pred2_object)
...
class GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_2_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_2_Action, self).__init__()
        self.name = "GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_2_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.activation = nn.PReLU()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
    ...
    def forward(self, g_list, goalVec, goalObjectsVec, a_list):
        ...
        for ind,g in enumerate(g_list):
            ...
            one_hot_pred1 = [0] * len(pred1_values); one_hot_pred1[ind_max_pred1] = 1
            one_hot_pred1 = torch.Tensor(one_hot_pred1).view(-1,1)

            # Predicting the second argument of the action
            pred2_input = torch.cat([final_to_decode, one_hot_action], 1)
            pred2_object = self.activation(self.q1_object(
                        torch.cat([pred2_input.view(-1).repeat(self.n_objects).view(self.n_objects, -1), self.activation(self.embed(self.object_vec)), one_hot_pred1], 1)))
            pred2_object = self.activation(self.q2_object(pred2_object))
            pred2_object = self.q3_object(pred2_object)
            pred2_object = torch.sigmoid(pred2_object)
...
class GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_3_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_3_Action, self).__init__()
        self.name = "GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_3_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.activation = nn.PReLU()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
    ...
    def forward(self, g_list, goalVec, goalObjectsVec, a_list):
        ...
        for ind,g in enumerate(g_list):
            ...
            pred1_input = torch.cat([final_to_decode, one_hot_action], 1)
            pred1_object = self.activation(self.p1_object(
                        torch.cat([pred1_input.view(-1).repeat(self.n_objects).view(self.n_objects, -1), self.activation(self.embed(self.object_vec)), h_old, metric_part], 1)))
            pred1_object = self.activation(self.p2_object(pred1_object))
            pred1_object = self.p3_object(pred1_object)
            pred1_output = torch.sigmoid(pred1_object)
            pred1_values = list(pred1_output.view(1,-1)[0])
            ...
            pred2_input = torch.cat([final_to_decode, one_hot_action], 1)
            pred2_object = self.activation(self.q1_object(
                        torch.cat([pred2_input.view(-1).repeat(self.n_objects).view(self.n_objects, -1), self.activation(self.embed(self.object_vec)), h_old, metric_part, one_hot_pred1], 1)))
            pred2_object = self.activation(self.q2_object(pred2_object))
            pred2_object = self.q3_object(pred2_object)
            pred2_object = torch.sigmoid(pred2_object)
...
class GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_4_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_4_Action, self).__init__()
        self.name = "GGCN_Metric_Attn_Aseq_L_Auto_Cons_C_4_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.activation = nn.PReLU()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
        ...
        self.metric1 = nn.Linear(in_feats, n_hidden)
        self.metric2 = nn.Linear(n_hidden, n_hidden)
        self.action_lstm = nn.LSTM(len(possibleActions) + n_objects + n_objects + n_states, n_hidden)
        self.n_hidden = n_hidden
        self.n_objects = n_objects
        self.n_states = n_states
        l = []
    ...
    def forward(self, g_list, goalVec, goalObjectsVec, a_list):
        a_list = [action2vec_cons(i, self.n_objects, self.n_states) for i in a_list]
        predicted_actions = []
        lstm_hidden = (torch.randn(1, 1, self.n_hidden),torch.randn(1, 1, self.n_hidden))
        goalObjectsVec = self.activation(self.embed(torch.Tensor(goalObjectsVec)))
        goal_embed = self.activation(self.embed(torch.Tensor(goalVec.reshape(1, -1))))
