...
class GGCN_metric_att_aseq_L_auto_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_metric_att_aseq_L_auto_Action, self).__init__()
        self.name = "GGCN_metric_att_aseq_L_auto_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
            self.layers.append(GatedHeteroRGCNLayer(n_hidden, n_hidden, etypes, activation=activation))
    ...
    def forward(self, g_list, goalVec, goalObjectsVec, a_list):
        ...
        goal_embed = self.activation(self.embed(torch.Tensor(goalVec.reshape(1, -1))))
        for ind,g in enumerate(g_list):
            h_graph = g.ndata['feat']
            for i, layer in enumerate(self.layers):
                h_graph = layer(g, h_graph)
            metric_part = g.ndata['feat']
            metric_part = self.activation(self.metric1(metric_part))
            metric_part = self.activation(self.metric2(metric_part))
            h = torch.cat([h_graph, metric_part], dim = 1)
            if (ind != 0):
                lstm_out, lstm_hidden = self.action_lstm(a_list[ind-1].view(1,1,-1), lstm_hidden)
            else:
                lstm_out = torch.zeros(1, 1, self.n_hidden)
            ...
            pred1_input = torch.cat([final_to_decode, one_hot_action], 1)
            pred1_object = self.activation(self.p1_object(
                        torch.cat([pred1_input.view(-1).repeat(self.n_objects).view(self.n_objects, -1), h_graph], 1)))
            pred1_object = self.activation(self.p2_object(pred1_object))
            pred1_object = self.activation(self.p3_object(pred1_object))
            ...
            pred2_input = torch.cat([final_to_decode, one_hot_action, pred1_output], 1)
            pred2_object = self.activation(self.q1_object(
                        torch.cat([pred2_input.view(-1).repeat(self.n_objects).view(self.n_objects, -1), h_graph], 1)))
            pred2_object = self.activation(self.q2_object(pred2_object))
            pred2_object = self.activation(self.q3_object(pred2_object))
...
class GGCN_metric_att_aseq_L_auto_tool_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_metric_att_aseq_L_auto_tool_Action, self).__init__()
        self.name = "GGCN_metric_att_aseq_L_auto_tool_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
            self.layers.append(GatedHeteroRGCNLayer(n_hidden, n_hidden, etypes, activation=activation))
...
class GGCN_metric_att_aseq_L_graph_auto_Action(nn.Module):
    def __init__(self,
                 in_feats,
                 n_objects,
                 n_hidden,
                 ...
                 activation,
                 dropout):
        super(GGCN_metric_att_aseq_L_graph_auto_Action, self).__init__()
        self.name = "GGCN_metric_att_aseq_L_graph_auto_Action_" + str(n_hidden) + "_" + str(n_layers)
        self.layers = nn.ModuleList()
        self.layers.append(GatedHeteroRGCNLayer(in_feats, n_hidden, etypes, activation=activation))
        for i in range(n_layers - 1):
            self.layers.append(GatedHeteroRGCNLayer(n_hidden, n_hidden, etypes, activation=activation))
